<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        const maxWidth = 1500;   // your profile picture max width
        const maxHeight = 1500;  // your profile picture max height

        document.getElementsByName("student_main_image")[0].addEventListener("change", function () {
            ...
            // 3. Check file type (must be an image)
            ...
            img.onload = function () {
                if (img.width > maxWidth || img.height > maxHeight) {
                    alert(`Image dimensions should not exceed ${maxWidth}x${maxHeight}px.`);
    ...
    function validateRegistrationForm() {
        ...
        for (let field of requiredFields) {
            if (!field) continue; // skip if element not found

            const value = field.value ? field.value.trim() : "";

            // ---- Check if empty ----
            if (value === "") {
                return showError(`Please enter ${field.getAttribute("name").replace(/_/g, " ")}`, field.id || field.name);
            }
            ...
            if (field.id === "date_of_birth") {
                ...
                if (isNaN(dob.getTime())) {
                    return showError("Invalid Date of Birth format", "date_of_birth");
                }
                ...
                if (age < 17) {
                    return showError("Age must be at least 17 years", "date_of_birth");
                }
                if (age > 50) {
                    return showError("Age must not exceed 50 years", "date_of_birth");
                }
            }

            if (field.id === "cnic_no") {
                if (!/^\d{13}$/.test(value)) {
                    return showError("CNIC must be exactly 13 digits", "cnic_no");
                }
            }
            ...
            if (field.type === "email" || field.id === "email") {
                ...
                if (!emailPattern.test(value)) {
                    return showError("Please enter a valid Email address", field.id);
                }
            }

            if (field.id === "mobile_1") {
                const cleanMobile = value.replace(/\D/g, ""); // keep digits only
                if (!/^\d{11}$/.test(cleanMobile)) {
                    return showError("Please enter a valid Mobile Number (e.g. 0399-1234567)", "mobile_1");
                }
            }

            const fileInput = document.getElementById("student_main_image")
                || document.querySelector('[name="student_main_image"]');
            if (fileInput && fileInput.files.length === 0) {
                return showError("Please select a student image to upload", fileInput.id || fileInput.name);
            }
    ...
    function validateDOB() {
        ...
        // Get birth year from date_of_birth field
        var birthDate = $('#date_of_birth').val();
        var birthYear = birthDate ? new Date(birthDate).getFullYear() : null;

        $("input[name='pass_year[]']").each(function () {
            var year = parseInt($(this).val());

            if (!isNaN(year)) {
                if (year > currentYear) {
                    toastr.warning("Passing year cannot be greater than current year (" + currentYear + ").", "Info:");
                    isValid = false;
                    return false; // break loop
                }
                if (birthYear && year < birthYear) {
                    toastr.warning("Passing year cannot be before your birth year (" + birthYear + ").", "Info:");
                    isValid = false;
                    return false; // break loop
                }
            }
        ...
        $("input[name='mark_obtained[]']").each(function () {
            ...
            success: function (response) {
                ...
                if (data.error) {
                    ...
                    if (data.semester.length > 0) {
                        ...
                        appendAcademicInfoRow(firstSemesterId);
