...
class SemesterController extends CollegeBaseController
{
    ...
    {
        $data = [];
        // $data['semester'] = Semester::select(`id`, 
        //     `semester`, `staff_id`,  `gradingType_id`, `semester_fee`, `major_subject_count`, 
        //     `status`, `f`.`faculty`, `f`.`faculty_code`)
        //     ->join('faculty_semester AS fs', 'semester.id', "=", 'fs.semester_id')
        //     -join('faculties AS f', 'fs.faculty_id', "=", 'f.id')->orderBy('semester')
        //     ->get();

        $data['semester'] = Semester::select(
            'semesters.id',
            'semesters.semester',
            'semesters.staff_id',
            'semesters.gradingType_id',
            'semesters.semester_fee',
            'semesters.major_subject_count',
            'semesters.status',
            'f.faculty',
            'f.faculty_code'
        )
        ->join('faculty_semester AS fs', 'semesters.id', '=', 'fs.semester_id')
        ->join('faculties AS f', 'fs.faculty_id', '=', 'f.id')
        ->orderBy('f.faculty_code')
