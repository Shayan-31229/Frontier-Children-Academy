...
class OnlineRegistrationController extends CollegeBaseController
{
    ...
    public function register(AddValidation $request)
    {
        //RegNo Generator Start
            ...
            $request->request->add(["reg_date" => Carbon::now()]);
            //reg generator End
        ...
        if ($request->hasFile('student_main_image')){
            ...
            $student_image->move(base_path('images/studentProfile').DIRECTORY_SEPARATOR, $student_image_name);
        }else{
            $student_image_name = "";
        }
        ...
        if ($request->hasFile('student_signature_main_image')){
            ...
            $student_signature_image->move(base_path('images/studentProfile').DIRECTORY_SEPARATOR, $student_signature_image_name);  //public_path().DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'studentProfile'
        }else{
        ...
        $parential_image_path = base_path('images/parents'); //public_path().DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'parents'.DIRECTORY_SEPARATOR;
        ...
        if ($request->has('board')) {
            foreach ($request->get('board') as $key => $board) {
                AcademicInfo::create([
                    ...
                    'created_by' => auth()->check() ? auth()->id() : 0, // ✅ fallback to 0 if not logged in
                ]);
            }
        }
    ...
    public function findSubject(Request $request)
    {
        ...
        if ($subjects && isset($numOfSubject)) {
            ...
            $response['error'] = 'No Subject Found. Please Contact Your Administrator.';
        }
    ...
    public function findRegistration(Request $request)
    {
        ...
        if($request->all()) {
            ...
            $data['student'] = Student::select('id', 'cnic_no','reg_no', 'reg_date','reg_fee','admission_course_fee as balance',
