    ...
    public function index(Request $request)
    {
        ...
        if ($request->all()) {
            ...
            $data['student'] = Student::select(
                ...
                ->whereIn("students.academic_status", [1, 2, 4])
                ->join('faculties as f', 'f.id', '=', 'students.faculty')
                ->join('semesters as s', 's.id', '=', 'students.semester')
                ->join('student_statuses as ss', 'ss.id', '=', 'students.academic_status')
        ...
        $data['academic_statuses'] = $this->activeStudentAcademicStatus();
...
//            }
            ...
            $password = str_replace('-', '', $student->cnic_no);
            ;
            ...
            if (!$alert) {
                ...
                $message = str_replace('{{email}}', $emailIds, $message);
                $emailMessage = str_replace('{{password}}', $password, $emailMessage);
                $emailMessage = str_replace('{{email}}', $emailIds, $emailMessage);
    ...
    public function view($id)
    {
        ...
        //attendance start
        $attendanceCollection = Attendance::select(
            'attendances.id',
        ...
        $subjectWiseAttendance = SubjectAttendance::select(
            'subject_attendances.id',
            'subject_attendances.subjects_id',
            'subject_attendances.attendance_type',
            'subject_attendances.link_id',
            'subject_attendances.years_id',
            'subject_attendances.months_id',
            'subject_attendances.day_1',
            'subject_attendances.day_2',
            'subject_attendances.day_3',
            'subject_attendances.day_4',
            'subject_attendances.day_5',
            'subject_attendances.day_6',
            'subject_attendances.day_7',
            'subject_attendances.day_8',
            'subject_attendances.day_9',
            'subject_attendances.day_10',
            'subject_attendances.day_11',
            'subject_attendances.day_12',
            'subject_attendances.day_13',
            'subject_attendances.day_14',
            'subject_attendances.day_15',
            'subject_attendances.day_16',
            'subject_attendances.day_17',
            'subject_attendances.day_18',
            'subject_attendances.day_19',
            'subject_attendances.day_20',
            'subject_attendances.day_21',
            'subject_attendances.day_22',
            'subject_attendances.day_23',
            'subject_attendances.day_24',
            'subject_attendances.day_25',
            'subject_attendances.day_26',
            'subject_attendances.day_27',
            'subject_attendances.day_28',
            'subject_attendances.day_29',
            'subject_attendances.day_30',
            'subject_attendances.day_31',
            'subject_attendances.day_32'
        )
        ...
        if ($data['lib_member'] != null) {
            $data['books_history'] = $data['lib_member']->libBookIssue()->select(
                'book_issues.id',
                'book_issues.member_id',
                'book_issues.book_id',
                'book_issues.issued_on',
        ...
        $data['hostel_history'] = ResidentHistory::select(
            'resident_histories.years_id',
            'resident_histories.hostels_id',
            'resident_histories.rooms_id',
            'resident_histories.beds_id',
            'resident_histories.history_type',
            'resident_histories.created_at'
        )
        ...
        */
        ...
        $data['schedule_exams'] = ExamSchedule::select(
            'exam_schedules.years_id',
            'exam_schedules.months_id',
            'exam_schedules.exams_id',
            'exam_schedules.faculty_id',
            'exam_schedules.semesters_id'
        )
...
//    "absent_practical" => 0
        ...
        /*Transport History*/
        $data['transport_history'] = TransportHistory::select(
            'transport_histories.id',
            'transport_histories.years_id',
            'transport_histories.routes_id',
            'transport_histories.vehicles_id',
            'transport_histories.history_type',
            'transport_histories.created_at',
            'tu.member_id',
            'tu.user_type'
        )
    ...
    public function scrutiny(Request $request)
    {
        ...
        return view(parent::loadDataToView($this->view_path . '.registration.scrutiny'), compact('data'));
    }
    ...
    public function interview(Request $request)
    {
        ...
        return view(parent::loadDataToView($this->view_path . '.registration.interview'), compact('data'));
    }
    ...
    public function meritlist(Request $request)
    {
        ...
        return view(parent::loadDataToView($this->view_path . '.registration.meritlist'), compact('data'));
    }
    ...
    public function confirmadmission(Request $request)
    {
        ...
        return view(parent::loadDataToView($this->view_path . '.registration.confirmadmission'), compact('data'));
    }

    public function saveinterviewmarks(Request $request)
    {
        $response = [];
        $response['error'] = true;
        ...
        if ($request->has('std')) {
            ...
            $updatedCount = DB::table('students')->where('id', $std["id"])->update(['academic_status' => 10, 'interview_marks' => $std["marks"], "interview_remarks" => $std["remarks"]]);

            $students = DB::table('students')->where('id', $std["id"])->first();

            if ($updatedCount > 0) {
                $response['ids'] = $updatedCount;
                $response['error'] = false;
                $response['success'] = 'Marks saved successfully.';
                $emailSuccess = $this->sendEmail($students->email, "Interview conducted", "Dear ". $students->first_name.", thank for your interview at FCA, peshawar. You are advised to view merit list at college regularly. Thanks you.");
            } else {
                $response['error'] = 'No student found for interview.';
            }
        } else {
    ...
    public function update(EditValidation $request, $id)
    {
        ...
        if ($row) {
            ...
            $institutions = $request->get('institution', []);
            $boards = $request->get('board', []);
            $rollNos = $request->get('roll_no', []);
            $passYears = $request->get('pass_year', []);
            $majors = $request->get('major_subjects', []);
            $marks = $request->get('mark_obtained', []);
            $maxMarks = $request->get('maximum_mark', []);
            $percentages = $request->get('percentage', []);
            $academicIds = $request->get('academic_info_id', []);

            foreach ($institutions as $key => $institution) {
                $academicInfoId = Arr::get($academicIds, $key);

                $data = [
                    'students_id' => $row->id,
                    'institution' => $institution,
                    'board' => Arr::get($boards, $key),
                    'roll_no' => Arr::get($rollNos, $key),
                    'pass_year' => Arr::get($passYears, $key),
                    'major_subjects' => Arr::get($majors, $key),
                    'mark_obtained' => Arr::get($marks, $key),
                    'maximum_mark' => Arr::get($maxMarks, $key),
                    'percentage' => Arr::get($percentages, $key),
                    'sorting_order' => $key + 1,
                ];

                if ($academicInfoId && $academicInfo = AcademicInfo::find($academicInfoId)) {
                    $data['last_updated_by'] = auth()->id();
                    $academicInfo->update($data);
                } else {
                    $data['created_by'] = auth()->id();
                    AcademicInfo::create($data);
                }
            }
...
//            }
    ...
    //Force Delete All History No Message
    public function delete(Request $request, $id)
    {
        ...
        //guardian info
        //guardian delete when the guardian have no other students. if he/she have any other students no need to delete the guardian record.
        $guardian = $row->guardian()->first();
        if (isset($guardian)) {
    ...
    public function findSemester(Request $request)
    {
        ...
        if ($request->has('faculty_id')) {
            ...
            if ($faculty) {
                ...
                $response['error'] = 'No Semester Assign on This Faculty/Program/Class.';
            }
        } else {
            $response['message'] = 'Invalid request!!';
        }
    ...
    public function findStudentsByStatus(Request $request)
    {
        $response = [];
        $response['error'] = true;

        if ($request->has('faculty_id') && $request->has('status')) {

            $students = DB::table('students')
                ->join('parent_details', 'parent_details.students_id', '=', 'students.id')
                ->leftJoin('domiciles', 'domiciles.id', '=', 'students.domicile_id')
                ->where('faculty', $request->get("faculty_id"))
                ->where('academic_status', $request->get("status"))
                ->select(
                    'students.*',
                    'parent_details.father_first_name',
                    'parent_details.father_middle_name',
                    'parent_details.father_last_name',
                    'domiciles.district as domicile_district'
                )
                ->get()->map(function ($student) {
                    $student->encrypted_id = encrypt($student->id);
                    return $student;
                });

            foreach ($students as $student) {
                $student->degrees = DB::table('academic_infos')
                    ->where('students_id', $student->id)
                    ->select(
                        'id as degree_id',
                        'mark_obtained as obtained_mark',
                        'maximum_mark as total_marks',
                        'board as degree_name' // replacing degree_name with board
                    )
                    ->get()
                    ->mapWithKeys(function ($item, $index) {
                        return [$index + 1 => $item]; // start keys from 1
                    });
            }

            if ($students) {
                if ($request->get("status") == 10) {
                    $maxStudents = DB::table('faculties')
                        ->where('id', $request->get('faculty_id'))
                        ->value('max_students');

                    $response['max_students'] = $maxStudents;

                    $studentsselected = DB::table('students')
                        ->where('faculty', $request->get('faculty_id'))
                        ->where('semester', 1)
                        ->whereIn('academic_status', [11, 1])
                        ->count();

                    $response['selected_students'] = $studentsselected;
                }

                $response['students'] = $students;
                $response['error'] = false;
                $response['success'] = 'Students Available For This Faculty/Program/Class.';
            } else {
                $response['error'] = 'No student found in this Faculty/Program/Class.';
            }
        } else {
            $response['message'] = 'Invalid request!!';
        }
    ...
    public function dropstudents(Request $request)
    {
        ...
        if ($request->has("stds")) {
            $stds = $request->get('stds');
            $updatedCount = 0;

            DB::transaction(function () use ($stds, &$updatedCount) {
                foreach ($stds as $std) {
            ...
            if ($updatedCount > 0) {
                $response['ids'] = $updatedCount;
                $response['error'] = false;
                $response['message'] = 'Selected Students dropped from meritlist.';
            } else {
                $response['error'] = 'No student found for dropping.';
            }
        } else {
    ...
    public function notifystudent(Request $request)
    {
        ...
        if ($request->has("stds")) {
            ...
            if ($updatedCount > 0) {
                $response['ids'] = $updatedCount;
                $response['error'] = false;
                $response['message'] = 'Students notified for date selected.';
    ...
    public function admissionconfirmed(Request $request)
    {
        $response = [];
    ...
    public function markforinterview(Request $request)
    {
        ...
        if ($request->has('ids')) {
            $ids = $request->get('ids');
            $updatedCount = DB::table('students')->whereIn('id', $ids)->update(['academic_status' => 9]);

            //$students = DB::table('students')->whereIn('id', $request->get('ids'))->get();

            if ($updatedCount > 0) {
                $response['ids'] = $updatedCount;
                $response['error'] = false;
                $response['message'] = 'Students marked for interview.';
            } else {
                $response['error'] = 'No student found for interview.';
            }
        } else {
    ...
    public function handleImportStudent(Request $request)
    {
        ...
        // 1️⃣ First pass — validate all rows
        foreach ($rows as $index => $row) {
            ...
            // Laravel validator for each row
            $validator = Validator::make($rowData, [
                'reg_no' => 'required  | max:25 | unique:students,reg_no',
                'reg_date' => 'required | date',
                'degree' => 'required | exists:faculties,id',
                //'domicile' => 'required | exists:domciles,id',
        ...
        // If any errors, stop here and return without inserting anything
        if (!empty($errors)) {
            return redirect()->back()->withErrors($errors);
        }

        // 2️⃣ Second pass — insert all rows in a single transaction
        DB::transaction(function () use ($cleanRows) {
            foreach ($cleanRows as $row) {
                ...
                $student = Student::create([
                    ...
                    "cnic_no" => $row['cnic_no'],
                    "kmu_reg_no" => $row['kmu_reg_no'],
                    "pnc_reg_no" => $row['pnc_reg_no'],
                ...
                if ($alert && $row['email']) {
                    //Dear {{first_name}}, We would like to inform you are successfully registered. Thank You.
                    $subject = $alert->subject;
                    $message = $alert->template;
                    $message = str_replace('{{first_name}}', $row['first_name'], $message);
                    $emailIds = [$row['email']];

                    // These lines are added by Shayan Khan 
    ...
    public function studentNameAutocomplete(Request $request)
    {
        if ($request->has('q')) {
            ...
            $students = Student::select(
                ...
                ->orWhere('ai.mobile_1', 'like', '%' . $request->get('q') . '%')
                ->orWhere('ai.mobile_2', 'like', '%' . $request->get('q') . '%')
                ->orWhere('students.academic_status', 'like', '%' . $request->get('q') . '%')
                ->join('addressinfos as ai', 'ai.students_id', '=', 'students.id')
                ->join('parent_details as pd', 'pd.students_id', '=', 'students.id')
                ->get();
            ...
            $response = [];
            foreach ($students as $student) {
                $response[] = ['id' => $student->id, 'text' => $student->reg_no . ' | ' . $student->first_name . ' ' . $student->middle_name . ' ' . $student->last_name . ' | ' . $student->father_name . ' | ' . $this->getSemesterById($student->semester) . ' | ' . $student->mobile_1];
            }
    ...
    public function editFee($id)
    {
        ...
        return view('admin.students.edit_fee', compact('student'));
    }
    ...
    public function updateFee(Request $request, $id)
    {
        ...
        return redirect()->route('admin.students.fee.edit', $student->id)
    ...
    public function generatePdf($id)
    {
        ...
        $data['student'] = Student::select(
            ...
            'students.reg_fee',
            'students.sbi_collect_no',
            'students.bank_ref_no',
            'students.payment_date',
            'students.university_enrollment_no',
            'students.admission_date',
            'students.admission_no',
            'students.admission_course_fee',
            'students.special_category',
            'students.weightage_claim',
