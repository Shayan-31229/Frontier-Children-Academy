...
class TransactionHeadController extends CollegeBaseController
{
    ...
    public function edit(Request $request, $id)
    {
        ...
        $data['tr_head'] = TransactionHead::select('id', 'tr_head', 'acc_id', 'status')->orderBy('tr_head','asc')->paginate(env('PAGINATION_LIMIT',$this->pagination_limit));
    ...
    public function view(Request $request)
    {
        ...
        if($request->all()){
            //$request->tr_start_date = date('Y-m-01');
            //$request->tr_end_date = date('Y-m-t');
            if($request->tr_head) {
                $request->merge([
                    'tr_start_date' => $request->get('tr_start_date', date('Y-m-01', strtotime('-1year'))),
                    'tr_end_date'   => $request->get('tr_end_date', date('Y-m-t'))
                ]);
