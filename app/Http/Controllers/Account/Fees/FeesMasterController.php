            ...
            if ($request->has('facility')) {
                ...
                if ($request->get('facility') == 1) {
                    $data['student'] = Student::select('students.id', 'students.reg_no', 'students.reg_date', 'students.first_name',
                        'students.last_name', 'students.faculty', 'students.semester', 'students.academic_status', 'students.status')
                        ->where(function ($query) use ($request) {
                            $this->commonStudentFilterCondition($query, $request);
                        })
                        ->where('l.user_type','=',1)
    ...
    try {
        foreach ($request->get('chkIds') as $row_id) {
            ...
            if ($row && $request->has('fee_head')) {
                foreach ($request->get('fee_head') as $key => $fee_head_id) {
                    $fee_amount = $request->get('fee_amount')[$key];
                    $fee_date   = Carbon::parse($request->get('fee_due_date')[$key])->format('Y-m-d');
                    $tr_date = Carbon::now()->toDateString();
                    // 1. Save FeeMaster

                    $alreadyIssued = FeeMaster::where('students_id', $row->id)
                        ->where('semester', $row->semester)
                        ->where('fee_head', $fee_head_id)
                        ->exists();

                    if ($alreadyIssued) {
                        $request->session()->flash($this->message_warning, 'Cannot issue the same fee to a student twice.');
                        return redirect()->route($this->base_route);
                    }
                    ...
                    $feeMaster = FeeMaster::create([
                        ...
                        'fee_due_date'  => $fee_date,
                        'fee_amount'    => $fee_amount,
                        'created_by'    => auth()->user()->id,
                    ]);

                    $accountsReceivableHeadId = 2; // ACCOUNTS RECEIVABLE
                    $revenueHeadId = 74;           // REVENUE
                    ...
                    Transaction::create([
                        ...
                        'date'            => $fee_date,
                        'tr_head_id'      => $accountsReceivableHeadId,
                        'dr_amount'       => $fee_amount,
                        'cr_amount'       => 0,
                        'description'     => 'Fee receivable from student ID ' . $row->id,
                        'status'          => 1,
                    ]);
                    ...
                    Transaction::create([
                        ...
                        'last_updated_by' => auth()->id(),
                        'date'            => $fee_date,
                        'tr_head_id'      => $revenueHeadId,
                        'dr_amount'       => 0,
                        'cr_amount'       => $fee_amount,
                        'description'     => 'Fee income for student ID ' . $row->id,
                        'status'          => 1,
                    ]);
        ...
        DB::rollBack();
        $request->session()->flash($this->message_warning, 'Transaction failed: ' . $e->getMessage());
        return redirect()->route($this->base_route);
    }
