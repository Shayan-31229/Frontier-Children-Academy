...
//             $group     = McqQuestionGroup::where('title',$row['groups'])->first();

//             $level     = McqQuestionLevel::where('title',$row['level'])->first();

//             if (!$subject) {
//                 return back()->withErrors("Subject code '{$row['subject_code']}' not found in database.");
//             }
//             if (!$group) {
//                 return back()->withErrors("Question group '{$row['groups']}' not found in database.");
//             }
//             if (!$level) {
//                 return back()->withErrors("Question level '{$row['level']}' not found in database.");
// }
//             $row['subjects_id'] = $subject->id;
//             $row['mcq_question_groups_id'] = $group->id;
//             $row['mcq_question_levels_id'] = $level->id;
...
public function handleImportQuestion(Request $request)
{
    ...
    foreach ($rows as $row) {
        ...
        // Fix malformed UTF-8 characters (important for auditing)
        ...
        if (!$subject) {
            $errors[] = "Row {$rowNumber}: Subject code '{$row['subject_code']}' not found.";
            continue;
        }
        if (!$group) {
            $errors[] = "Row {$rowNumber}: Question group '{$row['groups']}' not found.";
            continue;
        }
        if (!$level) {
            $errors[] = "Row {$rowNumber}: Question level '{$row['level']}' not found.";
            continue;
        }

        // duplicate question check
        if (McqQuestion::where('question', $row['question'])->exists()) {
            $errors[] = "Row {$rowNumber}: Question '{$row['question']}' already exists. Skipped.";
            continue;
        }
        ...
        if ($validator->fails()) {
            $errors[] = "Row {$rowNumber}: Validation failed for question '{$row['question']}'";
            continue;
        }
        ...
        $question = McqQuestion::create([
            ...
            'mcq_question_groups_id'  => $group->id,
            'mcq_question_levels_id'  => $level->id,
            "question"                => $row['question'],
            "mark"                    => $row['mark'],
            "type"                    => $row['type'],
            'created_by'              => auth()->user()->id
            ...
            for ($i = 1; $i <= 4; $i++) {
                if (!empty($row['option_'.$i])) {
                    McqQuestionOption::create([
                        ...
                        'mcq_questions_id'  => $question->id,
                        'option'            => $row['option_'.$i],
                        'answer_status'     => $row['option_'.$i.'_answer'] ?? 0,
                    ]);
                }
            }
        }
