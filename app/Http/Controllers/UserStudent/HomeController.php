...
class HomeController extends CollegeBaseController
{
    ...
    public function index()
    {
        ...
        $data['student'] = Student::select(
            ...
            'students.status',
            'student_statuses.preadmission',
            'student_statuses.title',
            'student_statuses.description'
        )
            ->leftJoin('student_statuses', 'students.academic_status', '=', 'student_statuses.id')
        ...
        $now = date('Y-m-d');
        $data['notice_display'] = Notice::select(
            'last_updated_by',
            'title',
            'message',
            'publish_date',
    ...
    public function editProfile(Request $request, $id)
    {
        ...
        $data['academic_statuses'] = array_prepend($academicStatus, 'Select Status', 0);
    ...
    public function library()
    {
        ...
        if ($data['lib_member'] != null) {
            ...
            $data['books_taken'] = $data['lib_member']->libBookIssue()->select(
                'book_issues.id',
    ...
    public function bookList(Request $request)
    {
        ...
        if ($data['lib_member']) {
            $data['book_request'] = BookMaster::select(
                'book_masters.id',
                'book_masters.code',
                'book_masters.title',
                'book_masters.image',
                'book_masters.categories',
                'book_masters.author',
                'book_masters.publisher',
    ...
    public function hostel()
    {
        ...
        $data['history'] = ResidentHistory::select(
    ...
    public function examSchedule(Request $request, $year = null, $month = null, $exam = null, $faculty = null, $semester = null)
    {
        ...
        $data['subjects'] = ExamSchedule::select(
            'exam_schedules.id',
            'exam_schedules.subjects_id',
            'exam_schedules.date',
            ...
            'exam_schedules.pass_mark_practical',
            's.code',
            's.title'
        )
            ->where([
        ...
        if ($data['subjects']->count() == 0)
            return back()->with($this->message_warning, 'No Subject Scheduled in your target exam. Please, Schedule exam first. ');
    ...
    public function admitCard(Request $request, $year = null, $month = null, $exam = null, $faculty = null, $semester = null)
    {
        ...
        if ($data['subjects']->count() == 0)
            return back()->with($this->message_warning, 'No Subject Scheduled in your target exam.');
    ...
    public function examScore(Request $request, $year = null, $month = null, $exam = null, $faculty = null, $semester = null)
    {
        ...
        $filteredStudent = $students->filter(function ($value, $key) use ($semester, $exam_schedule_id) {
            $subject = $value->markLedger()
                ->select(
                    'exam_mark_ledgers.exam_schedule_id',
...
//                    }
    ...
    public function addAnswer(Request $request, $id)
    {
        ...
        $assignment = $data['assignment'] = Assignment::select(
            'id',
            'created_by',
            'last_updated_by',
            'years_id',
    ...
    public function editAnswer(Request $request, $id)
    {
        ...
        //check available time period
        if (!$this->checkSubmitTimeLimitation($request, $assignment))
            return parent::invalidRequest();
    ...
    public function viewAssignmentAnswer(Request $request, $id, $answer)
    {
        ...
        $data['answers'] = $data['assignment']->answers()->where('assignment_answers.id', $answer)
            ->select(
                'assignment_answers.created_by',
                'assignment_answers.last_updated_by',
                'assignment_answers.id',
                'assignment_answers.answer_text',
                'assignment_answers.file',
                'assignment_answers.approve_status',
                'assignment_answers.status',
                's.id as students_id'
            )
...
//  "attach_file" =>
        ...
        $data = [];
        $data['student'] = Student::find($studentId);
        $folder_path = public_path() . DIRECTORY_SEPARATOR . 'application' . DIRECTORY_SEPARATOR;
