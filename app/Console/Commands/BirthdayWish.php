...
class BirthdayWish extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($students as $student) {
            $this->processBirthdayRecipient($student->first_name, $student->email, $student->contact_number, $alert);
        }
        ...
        foreach ($staffs as $staff) {
            $this->processBirthdayRecipient($staff->first_name, $staff->email, $staff->mobile_1, $alert);
        }
    }

    protected function processBirthdayRecipient($firstName, $email, $contactNumber, $alert)
    {
        $subject = $alert->subject;
        $message = str_replace('{{first_name}}', $firstName, $alert->template);

        // Send SMS
        if ($alert->sms == 1 && !empty($contactNumber)) {
            $numbers = $this->contactFilter([$contactNumber]);
            $this->sendSMS($numbers, $message);
        }

        // Send Email
        if ($alert->email == 1 && filter_var($email, FILTER_VALIDATE_EMAIL)) {
            dispatch(new AllEmail([$email], $subject, $message))
