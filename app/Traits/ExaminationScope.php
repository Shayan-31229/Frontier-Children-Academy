...
//                            }
                ...
                //verify both th & pr absent
                if($value->percentage > 0) {
                    $value->grade_point = $value->count() > 0
                    ? number_format($value->sum('grade_point') / $value->count(), 2)
                    : 0;
                    $value->final_grade = $this->getFinalGrade($semester, $value->grade_point);
                    $value->remark = $this->getRemark($semester, $value->percentage);
            ...
            $student->transcriptGPA = $transcriptCHS > 0
                ? number_format((float)$transcriptGradeWithCredit / $transcriptCHS, 2)
                : 0;
    ...
    public function getTranscriptData($studIds, Request $request)
    {
        ...
        if($groupBySemester->count() == 0)
        {
            $request->session()->flash($this->message_warning, 'There is no subject added, yet. Please add subject and then try again');
            return back();
        }
        //$semester = Semester::find($groupBySemester->pluck('semesters_id'));
        foreach ($groupBySemester as $key => $semesterLedger) {
