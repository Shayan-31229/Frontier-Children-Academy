...
trait StudentScopes{
    ...
    public function getStudentCNICNo($id)
    {
        
        $student = Student::where('id',$id)->first();
        if ($student) {
            return $student->cnic_no;
        }else{
            return "Unknown";
        }
    }
    ...
    {   
        ...
        if ($request->has('cnic_no')) {
            $query->where('students.cnic_no', 'like', '%' . $request->cnic_no . '%');
            $this->filter_query['cnic_no'] = $request->cnic_no;
        }
...
// FooBar
       ...
       $text = str_replace('{{date_of_birth_in_word}}', $dateOfBirthInWord, $text);
        ...
        $text = str_replace('{{cnic_no}}', $student->cnic_no, $text);
