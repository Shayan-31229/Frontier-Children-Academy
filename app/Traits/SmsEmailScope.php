...
use App\Traits\SmsGateway\TelenorSMS;
...
trait SmsEmailScope{
    ...
    {
        if($contactNumbers == "")
            return back()->with($this->message_warning, "No Contact Found. So Message Not Send In This Time. Please Try Again.");
        ...
        switch ($activeProvider){
            case "TelenorSms":
                $this->TelenorSMS($contactNumbers, $message);
                break;
            ...
            default:
                return back()->with($this->message_warning, "No SMS Service Provider Active. Please, Active First.");
    ...
    public function sendEmail($emailIds, $subject, $message){
        \Log::info("Email sent to " . $emailIds . " by local application");
        ...
        \Log::info("The email is sent from SMSEmailScope.php");
        /*Mail Queue*/
        ...
        public function sendInfoEmail($emailIds, $subject, $message){
        /*check internet connection for email sending*/
        $connection = Parent::checkConnection();
        if(!$connection)
            return back()->with($this->message_warning, $this->internet_status);
    ...
    public function emailFilter($emailCollections)
    {
        if(!empty($emailCollections)){
            ...
            if(!isset($emailIds)) {
                return back()->with($this->message_warning, "No Email Id Found. Please, Select Your Target With Valid Email Group");
            }
    ...
    public function checkSmsCredit(Request $request)
    {
        ...
        if($response->credits_available > 0){
            ...
            return back()->with($this->message_warning, "You Have No SMS Credit/".$response->credits_available." SMS CREDIT AVAILABLE");
        }
    }
